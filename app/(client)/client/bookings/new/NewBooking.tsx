    ...
    <div className="min-h-screen bg-[#fdf6f8]">
      ...
      <header className="sticky top-0 z-50 bg-white/80 backdrop-blur-xl border-b border-pink-100">
        <div className="max-w-2xl mx-auto px-4 pt-4 pb-3 flex items-center gap-4">
          ...
          <div className="flex-1">
            <h1 className="font-bold text-lg text-gray-900 leading-tight">Book Service</h1>
        ...
        <div className="max-w-2xl mx-auto px-6 pb-4">
          <div className="relative flex items-center justify-between">
            <div className="absolute left-5 right-5 top-4 h-0.5 bg-pink-100" />
            <div
              className="absolute left-5 top-4 h-0.5 bg-gradient-to-r from-[#e5849c] to-[#E5AFBC] transition-all duration-500"
              ...
              return (
                <div key={s} className="relative flex flex-col items-center gap-1.5 z-10">
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold transition-all duration-300 ${
                    done   ? 'bg-[#e5849c] text-white shadow-lg shadow-pink-200' :
                    active ? 'bg-white border-2 border-[#e5849c] text-[#e5849c] shadow-lg shadow-pink-100' :
      ...
      <main className="max-w-2xl mx-auto px-4 pt-6 pb-32 space-y-5">
        ...
        {currentStep === 1 && (
          <>
            {/* Date Picker */}
            <section className="bg-white rounded-3xl shadow-sm border border-pink-50 overflow-hidden">
                ...
                {dates.map((d, i) => (
                  <button
                    ...
                    className={`flex-shrink-0 w-[72px] py-3.5 rounded-2xl text-center transition-all duration-200 ${
                      selectedDate === d.full
                        ? 'bg-gradient-to-b from-[#e5849c] to-[#d4607a] text-white shadow-lg shadow-pink-200 scale-105'
                        : 'bg-gray-50 hover:bg-pink-50 border border-gray-100'
                    }`}
                  >
                    <div className={`text-[10px] font-semibold uppercase tracking-wide ${selectedDate === d.full ? 'text-pink-100' : 'text-gray-400'}`}>
                      {d.isToday ? 'Today' : d.day}
                    </div>
                    <div className={`text-2xl font-black my-0.5 ${selectedDate === d.full ? 'text-white' : 'text-gray-800'}`}>
                      {d.date}
                    </div>
                    <div className={`text-[10px] font-medium ${selectedDate === d.full ? 'text-pink-100' : 'text-gray-400'}`}>
                      {d.month}
                    </div>
                  </button>
                ))}
                        ...
                        {slots.map(({ time }) => (
                          <button
                            ...
                            className={`px-4 py-2.5 rounded-xl text-sm font-semibold transition-all duration-200 ${
                              selectedTime === time
                                ? 'bg-gradient-to-r from-[#e5849c] to-[#d4607a] text-white shadow-md shadow-pink-200'
                                : 'bg-gray-50 text-gray-600 hover:bg-pink-50 hover:text-[#e5849c] border border-gray-100'
                            }`}
                          >
                            {time}
                          </button>
            ...
            <div className="flex gap-3">
              {[
                ...
                <div key={b.text} className="flex-1 bg-white rounded-2xl p-3 text-center border border-pink-50 shadow-sm">
        ...
        {currentStep === 2 && (
          <>
                ...
                {apiAddresses.map((addr) => (
                  ...
                  >
                    <div className={`w-11 h-11 rounded-xl flex items-center justify-center text-xl flex-shrink-0 ${
                      selectedAddress.id === addr.id ? 'bg-pink-100' : 'bg-white'
                    }`}>
                      {addr.icon}
                    </div>
                    <div className="flex-1 min-w-0">
            ...
            <section className="bg-white rounded-3xl shadow-sm border border-pink-50 p-5">
        ...
        {currentStep === 3 && (
          <>
            {/* Hero Booking Card */}
            <section
              className="relative bg-gradient-to-br from-[#e5849c] via-[#d4718a] to-[#b85470] rounded-3xl overflow-hidden text-white p-5"
            >
              <div
              ...
              {!appliedCoupon ? (
                <div className="flex gap-3">
                  ...
                  <button
                    ...
                    disabled={!coupon}
                    className="px-5 bg-gradient-to-r from-[#e5849c] to-[#d4607a] text-white rounded-2xl text-sm font-bold disabled:opacity-40"
                  >
                    Apply
                  </button>
                </div>
        ...
        <div className="max-w-2xl mx-auto flex gap-3">
          {currentStep > 1 && (
            <button
              ...
              className="w-14 h-14 flex items-center justify-center rounded-2xl border-2 border-pink-200 text-[#e5849c] hover:bg-pink-50 transition-colors flex-shrink-0"
            >
              <ArrowLeft size={20} />
            </button>
          )}
          <button
            ...
            className="flex-1 h-14 rounded-2xl bg-gradient-to-r from-[#e5849c] to-[#d4607a] text-white font-bold text-base shadow-lg shadow-pink-200 disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-3 transition-all active:scale-95"
          >
            {currentStep === 3 ? (
              <>
